 ...
 */
var DingtalkSign = require('./dingtalk_sign');
...
var Api = function(conf) {
    this.suite_key = conf.suiteKey;
    this.suite_secret = conf.suiteSecret;
...
Api.prototype.getCorpToken = function(auth_corpid, timestamp, suite_ticket) {
    var self = this;
    const signer = new DingtalkSign(self.suite_secret);
    const signature = signer.getSignature(timestamp, suite_ticket);
    console.log('getCorpToken signature', signature);
    return new Promise(function (resolve, reject) {
        agent.post(BASE_URL + '/get_corp_token').query(
            {
        ...
        ).send({ auth_corpid: auth_corpid }).end(wrapper(resolve, reject));
    })
}
